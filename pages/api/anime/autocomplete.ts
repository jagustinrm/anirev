...
// Simple in-memory cache shared across requests in the same server process
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!q) return res.status(200).json({ data: [], pageInfo: null })

  const key = `autocomplete:${q}:${page}:${type}`
  ...
  try {
    const resp = await axios.post('https://graphql.anilist.co', { query, variables: { search: q, page, type } })
